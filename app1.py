...
if not os.path.exists('history'):
    os.makedirs('history')
model_map = {
    'FPH-CDet': 'runs/detect/train3/weights/best.pt',
...
with st.sidebar:
    ...
    selected = option_menu("功能列表", ["首页", "图片检测", "视频检测", "摄像头检测", "可视化计数", "历史检测"],
...
if selected == '首页':
    st.subheader("本平台是基于深度学习算法对图片、视频和摄像头三种输入实现对柑橘进行检测，图片与视频检测功能支持文件上传或URL输入形式进行检测，每次检测的结果都可在历史检测中查看")
