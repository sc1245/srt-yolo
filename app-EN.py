...
if not os.path.exists('history'):
    os.makedirs('history')
model_map = {
    'yolov7-tiny': 'runs/detect/train3/weights/best.pt',
}
...
with st.sidebar:
    ...
    with col1:
        st.image('logo.jpg')
    model_select = st.selectbox("Detection model selection", list(model_map.keys()))
    if model_select not in st.session_state:
        st.session_state[model_select] = YOLO(model_map[model_select])
    selected = option_menu("List of features", ["homepage", "Image detection", "Video detection", "Camera detection", "Historical detection"],
                           icons=['house-gear', 'image', 'film', 'webcam', 'hourglass-split'], 
...
if selected == 'Homepage':
    st.subheader("This platform is based on the YOLOv7-tiny algorithm to perform orange detection on three types of inputs: images, videos, and cameras. The image and video detection features support file uploads or URL input for detection. The results of each detection can be viewed in the history of previous detections.")
    ...
    if selection == options[0]:
        uploaded_file = st.file_uploader("Please upload the image to be detected.", type=["jpg", "jpeg", "png"])
        start_btn = st.button("Detect")
